...
# player pick a party Pokémon and redistribute its EVs.
#
...
class EVTrainingScene
  ...
  # Preview the actual stats these SP would produce (via a throwaway clone).
  def preview_stats
    test = @pkmn.clone
    ev = {}
    AREVTrain::STATS.each_with_index { |sid, i| ev[sid] = @sp[i] * AREVTrain::EV_PER_SP }
    test.ev = ev
    test.calc_stats
  ...
  def refresh
    ...
    nat = (@pkmn.nature ? @pkmn.nature.name : "-") rescue "-"
    tp.push([_INTL("능력 보정: {1}", nat), 32, y + 6, 0, BASE, SHADOW])
    tp.push([_INTL("좌우:조정  상하:선택  L:최소 R:최대  A:결정 B:취소"),
  ...
  def run
    ...
    loop do
      ...
      if Input.trigger?(Input::BACK)
        ...
        @sel = (@sel - 1) % AREVTrain::STATS.length
        ...
        pbPlayCursorSE if adjust(@sel, 1)
        refresh
      elsif Input.trigger?(Input::JUMPDOWN)   # L = MIN
        @sp[@sel] = 0
        pbPlayDecisionSE; refresh
