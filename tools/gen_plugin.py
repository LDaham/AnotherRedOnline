# -*- coding: utf-8 -*-
# Assemble plugin/AnotherRedOnline/[018] ChampionsBalance.rb.
# Method bodies containing Korean _INTL strings are pulled verbatim from the
# decompiled sources (never hand-typed) to avoid transcription errors.
import io, sys, re
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')

# --- 1) reproduced pbTryUseMove (already edited by gen_trymove.py) ------------
trymove = open('tmp/_trymove_gen.rb', encoding='utf-8').read().rstrip('\n')
# rename the method so we can alias it into the right slot
trymove = trymove.replace('def pbTryUseMove(', 'def arnet_champ_pbTryUseMove(', 1)
# indent each line by 2 more spaces (to sit inside `class Battle::Battler`)
trymove_indented = '\n'.join(('  ' + ln) if ln.strip() else ln for ln in trymove.split('\n'))

# --- 2) Rest sleep message line, pulled from source, 3 -> 4 -------------------
rest_src = open('decompiled/Scripts/181_MoveEffects_Healing.rb', encoding='utf-8').readlines()
rest_line = None
for l in rest_src:
    if 'pbSleepSelf' in l and '_INTL' in l:
        rest_line = l.strip()
        break
assert rest_line, 'Rest pbSleepSelf line not found'
rest_line4 = re.sub(r',\s*3\)\s*$', ', 4)', rest_line)
assert rest_line4.endswith(', 4)'), 'Rest line edit failed: ' + rest_line
...
out = HEADER.replace("__TRYMOVE__", trymove_indented).replace("__REST__", rest_line4)
open('plugin/AnotherRedOnline/[018] ChampionsBalance.rb', 'w', encoding='utf-8').write(out)
print('written; trymove lines=', trymove_indented.count(chr(10)) + 1)
print('rest line ->', rest_line4)
